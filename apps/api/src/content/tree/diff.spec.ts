...
describe('countChanges', () => {
  ...
  it('counts one per changed singleton leaf, per locale', () => {
    ...
    expect(changesAfter([set('home.en.pricingNote', 'x'), set('chrome.ru.navCta', 'x')])).toBe(2);
    expect(
      changesAfter([set('chrome.en.footerRights', 'x'), set('home.hiddenSections', ['works'])]),
    ).toBe(2);
  });
