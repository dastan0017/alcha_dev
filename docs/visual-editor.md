...
type throws `CmsPatchError` (API → 400). Business invariants (one featured
service, one highlighted plan) are expressed as explicit extra patches by the
client, so `applyPatches` stays dumb and `invertPatches` stays exact.
...
neutral field, bad/duplicate slug — `PROJECT_SLUG_PATTERN`) block with 422; blank
...
| `data-cms-preview-ui`                                                                                                          | root of a ghost slot or strip; hidden in view mode                                                                              |                                                                             |
...
neighbouring card shares the column, ←→ otherwise. The image scrim lets the
wheel through; only «Заменить фото» takes the pointer.
...
case pages, «Все работы» (the breadcrumb goes to `/`). JSON-LD omits the offers
of hidden `pricing` / `services`.
