...
| D17 | The «Цены» cards were rebuilt on 2026-09-22. A card reads top to bottom: tier label (`name`, rendered uppercase), price, `termLine` (timing only), the button, the «кому» line (`description`, set dark) with a grey «Например:» line (`examples` after the home label `pricingExamplesLabel`), a rule, then the feature list under `listHeading` (required) — the heading carries the tier ladder («Всё из «Лендинга», плюс:»), which used to be a first bullet nobody read as one. Optional add-ons stay a separate `extras` list rather than an `isOptional` flag per feature: the tree only has string lists, and optional rows always follow the included ones. They render with a grey «+», #6F6878 text and the `pricingOptionalLabel` prefix («По желанию:», also the screen-reader cue), never a ✓. One exclusive `highlighted` plan gets the purple frame, the primary button and the badge (`highlightLabel`: who the plan suits). Payment terms appear only in `pricingFootnote`. Layout: three columns ≥1024px, each card a five-row `subgrid` (price, button, «кому», rule, list) so buttons, rules and list headings line up and the cards end level; 640–1023px two columns with the highlighted plan first on its own row, its list beside its price; <640px one column in page order. Nothing hides behind a toggle: the phone carousel and its `pricingSwipeHint` chrome label are gone. Migration `20260922180000_pricing_ladder` writes the approved copy into the seeded plans (matched by RU name) and a stored draft, and gives any other plan the «Что входит:» heading and add-ons without a spelled-out «По желанию:». Copy rules for every later edit: no promise of leads or Google rankings, optional items never carry a ✓, the badge says who the plan suits rather than how popular it is, payment terms only in the footnote. | Approved design «Pricing Section Options» 1a and its spec: the tier ladder was invisible, and nothing said who each plan is for — the visitor had to map a feature list onto their own business. |
...
HomeCopy    = homeContentSchema                    // 30 fields (heroBullets: string[])
...
chrome all · steps `title description` · pricing `name priceLabel termLine ctaLabel description listHeading` ·
